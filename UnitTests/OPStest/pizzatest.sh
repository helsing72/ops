...
build.debug/UnitTests/OPStest/UnitTests/test-serialize-and-deserialize >> Unit_test_results/UnitTests-result.txt ; count=$(($count + $?))
...
count=$(( $count + $exit_normal + $exit_tcp  + $exit_udp ))
