...
namespace ops
{
	...
	bool isValidNodeAddress(std::string addr)
	{
		//std::cout << "isValidNodeAddress(): " << addr << std::endl;
		if (addr == "") return false;
		unsigned long Ip = boost::asio::ip::address_v4::from_string(addr).to_ulong();
	...
	bool isMyNodeAddress(std::string addr, IOService* ioServ)
	{
